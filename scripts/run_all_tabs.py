...
def dedupe_rows(rows: List[Dict]) -> List[Dict]:
    ...
    out = []
    for r in rows:
        key = r.get("id") or r.get("redirect_url") or (r.get("title"), r.get("company"), r.get("location"))
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
def run_tab(tab: str, queries: List[str], days: int, per_page: int, max_pages: int, country: str, sheet: bool, sleep: float):
    print(f"[run] tab={tab} queries={queries} days={days} per_page={per_page} pages={max_pages}")
    all_rows: List[Dict] = []
    for q in queries:
        rows = list(fetch_jobs(
            country=country, what=q, where="", days=days,
    ...
    if sheet:
        ...
        publish_to_google_sheets(all_rows, worksheet_title=tab, keywords=tokens, columns=BASIC_COLUMNS)
...
def main():
    ...
    ap.add_argument("--sleep", type=float, default=0.4, help="Seconds between subqueries/tabs")
    ...
    tabs: Dict[str, Union[str, List[str]]] = {
        ...
        "appsec": [
            ...
            "devsecops application security", "sast dast code scanning", 'threat modeling "secure sdlc"'
        ],
        "cloudsec": [
            '"cloud security"', '"cloud security engineer"', "iam aws security",
            "kubernetes security", "cnapp cspm cwpp"
        ],
    }
